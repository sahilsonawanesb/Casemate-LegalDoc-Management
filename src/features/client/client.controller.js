...
export const createClient = async(req, res, next) => {
        try{
            ...
            if(!name || !email || !phone){
             ...
             const exisitingClient = await clientRepository.exisitingClient(email, attorneyId);

            // now check if the client is already created or not..
            if(exisitingClient){
                return next(new ApplicationError("Client with this email id already exisits", 409));
            }
